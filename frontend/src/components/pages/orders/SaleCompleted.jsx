  ...
  return (
    ...
    >
      ...
      <div className="w-full h-full p-4">
        <h2 className="text-2xl font-light text-left mb-4">Customer Completed Orders</h2>
        <table className="w-full border-collapse">
          ...
          <tbody className="font-light">
            {Array.isArray(sellerOrders) && sellerOrders.filter(order => order.status === 'completed').length > 0 ? (
              sellerOrders.filter(order => order.status === 'completed').map((order) => (
